...
> 更新日期：2026-09-22（v2.10 幕三空间账 + 打字机占位化 + 走线归位 · 浏览器实测取证）
...
### 2026-09-22 · v2.10 幕三空间账 + 打字机占位化 + 走线归位（浏览器实测驱动）

**取证方式**：本地 `http.server` + Chrome Headless Shell（CDP），1440×900 / 390×844 / `prefers-reduced-motion` 三档逐幕取盒位与投影标签坐标，非目测。

**A 幕三空间账（七行轴名回归）**
- 现象：1440×900 下 `STR-C`／`STR-S` 两行轴名被当幕文字框吃掉——投影守卫只能藏标签，不能把标签挤进正文，等于 42 维棋盘少了 2/7 根轴。
- 处置：新增 `boardFit`（平台期 p 0.54→0.66 起效、0.76→0.82 淡出）把相机后撤 3.4 单位、视点下探 1.9 单位，整盘缩小约 15% 并上移；`.act` 底边 12vh→10vh，`#act-3` 单独 8vh。
- 复验：可见轴标签 6 列 + 7 行 = **13/13**，压导航 0、压正文 0；列标签顶 71px（导航下沿 51 + 守卫 6）。

**B 打字机改占位揭示（修版式跳动）**
- 现象：逐字 `textContent +=` 每字符重排一次，幕框按 `bottom` 定位 → 行数增长把整块文字往上顶，边读边跳；屏读器只能读到当前残句。
- 处置：一次写入全文、按字切 opacity（`.tw` / `.tw.is-on`），节奏走 rAF 累计 dt（32ms/字）取代 `setTimeout`；减弱动效下不建打字机，正文按 HTML 原样常驻。
- 复验：`#act-2` 幕框高度打字前后恒为 154px（原逐字增长），已亮字数 3→31 而盒型零变化。

**C 数据流走线归位（修"渲染破口"观感）**
- 现象：走线 x 原取 `视口宽×0.94−10`，正落在层指示器面板内部（面板 z20 盖 svg z15），只在三个条目缝隙露几段虚线，看着像坏掉的残线；端点 y 又取 0.3/0.5/0.7 视口高，与指示器条目真实位置无关。
- 处置：端点改取 `.rail-item` 真实中心，走线贴指示器左缘外 14px，随 resize 重量（`measureRail`）。

**D 终幕棋盘退场提前（修节点压导航）**
- 现象：相机自 p≈0.72 起下潜，棋盘在屏幕上整体上移，而 recede 窗口原为 0.82→0.92，于是 p 0.82-0.90 段亮橙节点与纤维束一起压在顶栏导航上。
- 处置：recede 窗口提前至 0.76→0.86，连线退至 ~20%、节点退至 ~28%（原 40%/50%），收尾再幽灵化至 ~3%/~8%——终幕"42 维并未消失"仍留可辨幽灵，只是不再抢主体。
- 复验：p=0.857 档压导航标签计数 0。

**E 零尺寸绘冲区守卫（修 GL 报错刷屏）**
- 现象：容器尺寸为 0 时（内嵌浏览器未出图、隐藏标签页、0 高容器），每帧 `glClear` + 十余条 `glDraw*` 全报 `GL_INVALID_FRAMEBUFFER_OPERATION: Attachment has zero size`，实测单页累积 160 条告警且白耗 CPU。
- 处置：出图统一走 `drawFrame()`，`clientWidth/Height < 2` 即跳过；resize 同档守卫，0 尺寸时保留上一次有效绘冲区。
- 复验：1440×900 与 390×844 两档 GL 告警 0 条、控制台错误 0 条；模块脚本 `node --check` 通过。

**F 可及性两处**
- 公式芯片是可聚焦链接却挂 `aria-hidden="true"`（AT 读不到、仍在 Tab 序内）→ 去 aria-hidden，补 `aria-label` 说明"6 模块 × 7 维度"。
- 节点详情关闭钮与移动端提示条关闭钮由 `<span>` 改原生 `<button>`（可 Tab 可回车，Esc 关闭仍保留），并抹平 UA 默认盒型。

**G 竖屏取景与窄屏遥测（390×844 实测）**
- 现象：竖屏 6 列棋盘两侧被切、整列轴标签掉出视口，且末两行节点压在幕框下沿；底部遥测五项换行成两排，与幕框争同一条底带。
- 处置：`narrowFit`（aspect 1.2→0.35 线性）沿视线后撤 6 单位 + 幕三抬升量按窄屏加 1.3 单位；后撤只补到"棋盘可读"这一档——实测把整盘含轴标签都塞进竖屏需 22 单位，节点会缩到 8px 反而读不了，故竖屏宁可两侧出血。`≤768px` 整条 `#status-bar` 收起。
- 复验：390×844 档七行节点全部抬到幕框之上、节点清晰可读；轴标签在该档不可容纳（M1-M6 与七维名由幕框正文承载，不丢信息）。桌面 1440×900 档 `narrowFit=0`，与上一批逐帧一致。

**H 终幕纤维与遥测换行（800×500 截图取证）**
- 纤维束满不透明度把脑体糊成草垛：`dtiFiberOpacity` 上限 1.0→0.72，脑壳体积与通道色才分得开。
- 底部遥测在矮视口下折成两排并压进幕框：`#status-bar` / `#telemetry` 加 `flex-wrap: nowrap`。
- 曾试把终幕雾密度 0.008→0.02 求纵深，截图对比收益不成立且真机有压暗风险，已回退（保留回退理由于代码注释）。

**I 字号阶梯：塌陷免疫 + 全站层级归位（主理人 2026-09-22 指出"现象层标题太小"）**
- 根因不是比例：`--type-*` 全是裸 `var(--text-*)`，brand.css 一断（见 commit 3318301）整条阶梯 IACVT，`font-size` 退回继承 16px——幕标题与小标同为 16px，看起来就是"标题没放大"。现九档别名逐条加字面量兜底，实测把 brand.css 整条拦掉后 128/40/20/16/13/11.5 与正常档**逐字相同**，塌陷不再静默发生。
- 页内并级修掉：`--type-mono` 原与 `--type-mono-xs` 同为 2xs（badge、公式芯片与调试遥测一个尺寸）→ 提到 `--text-xs`；节点详情内 `.nd-link`/`.nd-markers` 11.5→14（行动链接原低于正文），`.nd-code` 13→11.5；移动端 act-h2 原绕过别名层直写 `--text-2xl`，改回 `--type-h2` 单处定义；`.mb-close`/`.nd-close` 三处硬编码入梯。
- 全站层级倒挂（子代理改、本窗逐条复核 diff 与实测）：六域页与 glossary、textbook 的 `.section-label`／`.section-heading` 是带序号＋图标＋下划线的**区块主标题**（非 eyebrow），原 16px 低于其自身 h3 18.4／h4 19.2 → 提到 `--text-xl` 24px，≤480px 档 20px；`shared/brand.css` h3 1.15rem→`--text-lg` 20px、各页 h4 1.2/1.05rem→`--text-md` 18px。iods 实测阶梯现为 h1 32 > 区块 24 > h3 20 > h4 18 > 正文 16，逐档单调。

**J 幕内可点件全废 + 重看按钮卡死（交互面逐条实测）**
- 严重度最高的一条：`.act { pointer-events: none }`（为的是面板不吞滚轮与拖拽）从未对框内控件放行，
  实测幕框 `is-live` 且 `visibility: visible` 时，`.badge-42`／`#replay-btn`／`.end-link`／`.legend-item`
  的命中测试全部打到 `CANVAS#gl`、计算值 `pointer-events: none`——即"42/42 徽章、重看 4 幕、
  反馈体验、三通道外链"对鼠标用户**一个都点不动**（程序 `.click()` 能触发，所以此前从未暴露）。
  补 `.act.is-live a, .act.is-live button { pointer-events: auto }`，复测四件全部命中自身。
- 重看按钮：监听确实触发、`scrollTo({top:0,behavior:'smooth'})` 也确实调用，但 8667px 的长距平滑回滚
  在重帧下卡在 8601 处 7s 不动（同页 rail／键程的短距平滑跳 0.30/0.58/0.88 全部正常落位）；
  改 `behavior: 'instant'`，复测 8667→0 且遥测 ACT 回到 1。
- 未测路径补齐：键盘 1-4 落位 0/0.30/0.58/0.88 与 ACT 1-4 对齐；三层指示器点击逐条落位；
  无 WebGL 时 `#fallback` 出图、canvas 隐藏、页面仍可滚、异常 0；`?prod=1` 收起遥测/光标/噪点/滚动提示而保留导航，异常 0。

**K 触摸档放开棋盘拖拽 + 投影 QA 钩子（390×844 合成触摸实测）**
- 原状：`pointerdown/pointermove` 两处 `if (coarsePointer) return`，触摸用户既不能转棋盘，
  而棋盘在竖屏只把正对镜头的一角放进来——实测**可点节点 12/42**，其余 30 个（含 M1／M6 整两列）永远够不到。
- 处置：放开触摸档拖拽，轴锁交给合成器——`@media (pointer: coarse) #gl { touch-action: pan-y }`
  （横滑转棋盘、竖滑照常翻页，比 JS 里判方向稳）；`mousePX` 不再作为点击前置（轻点常常没有 pointermove）。
- 复测：横滑后可点节点 12→16 且松手不回弹；竖滑 scrollY +305 正常翻页；轻点仍出 `M1 × DSP-F` 详情面板；异常 0。
- 残留上限（非本窗引入）：`tRY = clamp((mx-0.5)*ROT_Y_MAX + dragY, ±ROT_Y_MAX)` 是"透视倾斜"而非转台，
  且幕三棋盘已是平面，故 16/42 是该交互的设计天花板。要 42/42 可达只有两条路——把 `narrowFit` 后撤从 6 单位加到
  ~11 单位（节点从 ~24px 缩到 ~19px，换全列可点），或给竖屏加横向平移手势（新交互）。本窗按"不加新交互"停在 16/42，候主理裁。
- 另记：为写可复现的触摸用例，`window.__intreAct4` 增 `nodeScreen(col,row)`（返回该节点当前投影屏幕坐标），
  与既有 `setFiberOpacity` 等 QA 钩子同族。

**L 42 节点内容全量核 + 死链清零（可点件可达性全量扫）**
- 42 个节点详情逐条开合实测：标题 42/42 不重、正文 73-99 字、生物标记 20-52 字，无一条落回"数据加载中"兜底。
  （首轮统计报"8 条空"是**本窗判据写错**——`code.includes('× R')` 把 `× RES-B` 一起误判，复核后为 0 条。）
- 可点件可达性全量扫（每个 `a/button/[role=button]/[tabindex]` 在四幕位置做中心点命中测试）：**线上 6 个不可点**
  （幕三徽章 1 + 幕四"重看 4 幕""反馈体验"与三通道外链 5，全部命中到 canvas），本地修复后 **0 个**。
  同一扫描顺带清掉节点详情"教材详解"死链：原 `href="#"` + `target="_blank"` 会开一个指向本页顶部的多余标签页，
  改指教材总目录（与 `badge-42` 同一 URL，不虚构分章映射）；全仓 `href="#"` 现为非归档件 0 命中。
- 幕一星点密度：文档记 400（v2.1 Q8 以 LCP 为由下调），代码实为 `RANDOM_N=1500`，已把文档数字同步到代码并说明
  点云不入 LCP 关键路径。开场画面仍偏空，但**像素级量化失败**——WebGL 未开 `preserveDrawingBuffer`，
  `drawImage(canvas)` 取到空图；要么开该选项（有性能代价）要么改用 `readPixels` 时机控制。
  按"不盲调艺术参数"原则，密度与分布候主理目测后再定，本窗不动。

**未做／候裁**：① 幕三"棋盘 + 轴标签"在视口高 ≤820px 时仍无解，届时须改布局而非继续缩盘；② 终幕纤维束在软件渲染（SwiftShader）下 FPS 4-11，真机 GPU 帧率未测；③ 幕四终幕竖屏取景未逐帧目测（截图在 SwiftShader 下超时），仅按 narrowFit 连续性推断。
